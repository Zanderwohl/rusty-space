...
async fn a_craft_written_to_the_store_comes_back_bit_for_bit() {
    ...
    let row = save(&craft, Some("acct-checkpoint"), 900_000_000);
...
async fn the_newest_checkpoint_is_the_one_that_comes_back() {
    ...
    save_ships(&client, &[save(&craft, Some("acct-again"), 1)]).await.unwrap();
    craft.motion.clock_s = 999_999.0;
    save_ships(&client, &[save(&craft, Some("acct-again"), 2)]).await.unwrap();
