...
| `--beauty` / `--beauty-kind <kind>` | turn on the beauty shots, and hold them on one kind of subject (`whole`, `horizon`, `nadir`, `approach`, `destination`, `survey`, `star`, `field`). The first is taken a second in, so `--frames 240` photographs it |
