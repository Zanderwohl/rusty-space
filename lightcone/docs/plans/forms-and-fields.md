...
The build of [28-ship-form.md](../28-ship-form.md), [29-the-field.md](../29-the-field.md),
[30-directed-energy.md](../30-directed-energy.md) and [31-ship-rendering.md](../31-ship-rendering.md),
...
- read: 28 §Parts, §Kinds, §The Mind, §Placement is relative
...
- read: the Balance tables of 28, 29 and 30
...
- read: the Protocol sections of 28, 29 and 30
...
- read: 28 §Spars conform, §What the server computes
...
- read: 28 §Kinds, §Hull structure follows area
...
- read: 28 §The starting form, §Your own presets
...
- read: 28 §What the server computes from a form
...
- read: 28 whole, 19 for what stays
...
- read: 28 §What the server computes, 20 §Attitude
...
- read: 28 §Protocol and persistence
...
- read: 29 §The heat account, §Conversion, §Clear and Black
...
- read: 29 §The anchors
- deliver: `q_idle`, `field_capacity` and `τ` derived in `Balance::DEFAULT` from the starting form's envelope; 29's tables recomputed for it.
...
- read: 29 §The inputs, 30 §The star's gain
...
- done when: a full ship at 0.1 AU settles at 29's temperature, and a vent raises `Q` by exactly what storage could not hold.
...
- done when: a Black ship vanishes in V and not at ten microns, and a hot field's color moves through the bands as 29's table says.
...
- read: 30 §Spread, §What arrives, §Courtesy
...
- done when: 30's tables come out of the functions.
...
- read: 30 §The drive is the radiator
...
- read: 30 §Emitting on purpose, §Three uses, §Radio
...
- read: 30 §Exhaust lands on whatever is behind
...
- read: 30 §Maneuvering near others, 08 §Intercept
...
- read: 31 §From distance field to mesh
...
- read: 31 §Details are sized in meters, §Materials by kind
...
- read: 31 §Building, as a function of time
...
- read: 31 §A build step is a frontier
...
- read: 31 §The field, 29 §What an observer sees
...
- read: 28 §Handles, §Snapping, §What else it shows
...
- read: 28 §The budget, §What else it shows
...
- deliver: 03, 13 and 19 updated for what was built; 28 to 31 marked built, with what departed from the plan said where it did; the README's status.
