...
| interactivity | htmx, pinned and vendored | listed below; it is a small list |
...
| `/static/:hash/*` | site CSS, fonts, the htmx build | no |
...
## HTMX earns its place in four spots

Blog pagination, tag filtering, the newsletter form, and — later — the account pages the auth
service will bring. That is the whole list. Everything else is an anchor tag. A marketing site
that needs a hydration step to render a paragraph has lost the plot.

Two rules:

- **Vendor the exact build.** Serve it from `/static/:hash/htmx.js` with the site's own
  fingerprinting, not from a third-party CDN by floating tag. The one script permitted to
...
FROM chef AS builder              # SQLX_OFFLINE=true cargo build --release -p lc-web
...
Under 100 MB. Content is baked in, which is what makes a content rollback the same operation
as a code rollback. Site static files are fingerprinted at startup and served with a one-year
immutable header from the binary's own `/static/:hash/` route — the site is one image plus
environment, and does not depend on the CDN to render.
...
**Deliver:** `web/lc-web`, axum, maud, the static routes, health checks, the Dockerfile, and
one deploy to a real host.

**Done when:** `docker run -e BIND_ADDR=... <image>` serves the landing page with no other
configuration; the image is under 100 MB; SIGTERM drains in-flight requests; the separate-
workspace grep is in CI.

**Do not:** add a database, htmx, or a build pipeline.
...
rather than at request time; drafts are invisible in production and listed in staging.
