...
**A build carries its own sky.** `build-wasm.sh` packs `CATALOG` to
`assets/sky/catalog.lcsky` inside the build, and the client asks for exactly that path — there
is no URL parameter for it. To ship a different catalog, change `CATALOG` in the script and
commit, so the build id names the catalog as well as the code.

**Promoting a build is also repointing the shard**, when the catalog changed: its `--sky`
...
```bash
tools/publish-books.sh --check         # the shelf matches its catalog; needs no CDN
tools/publish-books.sh                 # uploads what the catalog names, to library/
```
...
The check is worth running on its own. It hashes every file the catalog names and compares it
...
docker --context rocinante run -d --name lightcone-shard --restart unless-stopped \
    ...
    --sky http://lightcone-cdn:3101/game/<promoted-build>/assets/sky/catalog.lcsky
```
...
`http://lightcone-cdn:3101/game/<build>/assets/sky/catalog.lcsky`. Not the public
...
against the same shell radius, so two catalogs is two answers to which system a ship is in —
...
The sky is not handed over at all. The client loads `sky/catalog.lcsky` from its build's asset
base, so a build served from anywhere — the CDN, or a staged directory on disk — draws the
catalog it was built with.
...
| the client starts and the sky is three stars | `assets/sky/catalog.lcsky` 404'd under the build; the client says so and falls back to a sample |
