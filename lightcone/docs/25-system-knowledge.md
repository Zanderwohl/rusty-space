...
| 6 | ✅ **`Part` grew `colors: Vec<Colors>`,** so the digest a craft folds actually leaves it. It never did: `Colors` was written, stored and read locally and was in no report, so a probe's months of photometry died with the probe and the earlier `REPORT_FORMAT` bump credited to it changed nothing on the wire. `Colors` itself changed shape at the same time -- it folds per-visit band *ratios* rather than raw fluxes, because raw flux carries the range and the phase of each visit and averaging it measured the geometry rather than the body. `FILE_FORMAT` is 9 and `REPORT_FORMAT` is 7 |
