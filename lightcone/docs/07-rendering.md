...
**Every giant is one graph, painted from its chemistry.** `worlds/giant.tgraph` (the `[giants]`
entry of the manifest, generated by texture-graph's `planets.rs`) takes a band count, contrast,
turbulence, storms, polar haze, a great spot and five colors, and `lc_world::giant` derives all
of them from what the giant is:

- **Cloud-top temperature picks what condenses**, after Sudarsky, Burrows and Hubeny (2000):
  ammonia ice below about 150 K, water to 350, nothing to 800 — a clear column, Rayleigh-blue —
  then sodium and potassium vapor that swallow the visible, and past 1400 K silicate cloud. The
  regimes blend smoothly, so no two giants either side of a threshold look like two classes.
- **The envelope's metals** are its star's `[Fe/H]` raised by `4 (M/M_J)^-0.9` — Jupiter four
  times solar, Saturn ten, the ice giants near eighty. A core that reached runaway is held under
  twelve and one that did not over thirty, which keeps a twenty-Earth gas giant apart from an ice
  giant by color. Metals set the methane over the deck, so how blue it is and how dark in K, and
  how deep the chromophores stain its belts. The generator states them as a `Metals:` tag.
- **Ultraviolet** — the equilibrium temperature's fourth power times the star's far-UV share —
  sets how fast chromophores and haze are made: a hot star stains the belts, a red dwarf leaves
  them pale, and far out there is little polar haze.
- **Spin** sets the band count, as the square root of how fast the equator turns (Rhines): a
  tidally locked hot Jupiter has two broad bands, Jupiter about six.

Each layer — zone, belt, storm, polar haze — has its own reflectance in every band, and the
rule, given the four solar giants' masses and temperatures, lands on their measured colors; a
test holds it there. The graph's layers are baked as masks and the shader mixes the layers'
spectra by them exactly as it mixes a rocky world's grounds, so a belt, a gap down to deeper
cloud under more methane, is dark in K where a storm's high tops are bright, and a belt is what
glows in the infrared. A survey reads the same spectra averaged by the share of the disc each
layer covers, which a test measures off the graph.

The cubemap is the look rather than the measurement. Jupiter's belts are within a few hundredths
of gray, so chroma is raised and darker layers pulled further below the zone, as every
photograph does; and the natural mapping's Sun is `(1.37, 1, 1.04)`, which drew a pale cyan
Uranus lavender, so the paint is balanced to the Sun. The spectra are untouched.

texture-graph's two evaluators part outside `[0, 1]`: the GPU clamps a Map's value and the CPU
does not, and the CPU caps a Multiply at one and the GPU does not. The graph keeps its ramps and
products inside, and a test bakes its masks and holds them to the CPU.

![Giants](../images/giants.png)

Jupiter, Saturn, Uranus and Neptune; then, worn on Jupiter's sphere with `--wear`: Caph g (56 K),
Menkent j (metal-poor), Alchiba i and Castor f (under hot stars, stained deep), Talitha i (an ice
giant an A star stains), Rasalhague f, Xami b (570 K, cloudless) and Chara b (986 K, alkali).
